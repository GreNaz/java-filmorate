...
public class FilmService {
    ...
    public List<Film> getFilmsByDirectorWithSort(int id, FilmSortBy sortType) {
        ...
        List<Film> films = filmStorage.getFilmByDirector(id);
        genreStorage.load(films);
        directorStorage.load(films);
        if (sortType.name().equals(FilmSortBy.year.name())) {
            return films.stream()
                    .sorted(Comparator.comparing(Film::getReleaseDate, Comparator.nullsLast(Comparator.naturalOrder())))
                    .collect(Collectors.toList());
        } else if (sortType.name().equals(FilmSortBy.likes.name())) {
            return films.stream()
    ...
    private List<Film> getByTitle(String query) {
        log.info("Getting films by search in Titles");
        List<Film> films = filmStorage.searchByTitle(query).orElseThrow(()
    ...
    private List<Film> getByDirector(String query) {
        log.info("Getting films by search in Directors");
        List<Film> films = directorStorage.search(query);
        genreStorage.load(films);
        directorStorage.load(films);
        return films;
    }
